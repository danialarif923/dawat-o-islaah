...
const NearestMosqueMap = () => {
  ...
  const searchMosques = useCallback(async (location, searchRadius, query = "") => {
    ...
    try {
      ...
      // Progressive display: add results in batches
      const gen = ++searchGenRef.current;
      const batchSize = 15;
      let index = 0;
      setProgressTotal(mosquesWithDistance.length);
      setProgressLoaded(0);
      const addBatch = () => {
        if (gen !== searchGenRef.current) return;
        const next = mosquesWithDistance.slice(0, index + batchSize);
        setMosques(next);
        setFilteredMosques(next);
        index += batchSize;
        setProgressLoaded(Math.min(index, mosquesWithDistance.length));
        if (index < mosquesWithDistance.length) {
  ...
  // Handle radius changes after manual area search (Enter key)
  useEffect(() => {
    const searched = searchedLocationRef.current;
    if (!searched) return;
    setLoading(true);
    setError(null);
    ...
    (async () => {
      try {
        const results = await searchMosquesViaOverpass(searched, radius, "", abortRef.current.signal);
        const withDist = results.map(m => ({
          ...m,
          distance: calculateDistance(searched.lat, searched.lng, m.lat, m.lng),
        }));
        const gen = ++searchGenRef.current;
        const batchSize = 15;
        let idx = 0;
        setProgressTotal(withDist.length);
        setProgressLoaded(0);
        const addBatch = () => {
          if (gen !== searchGenRef.current) return;
          const next = withDist.slice(0, idx + batchSize);
          setMosques(next);
          setFilteredMosques(next);
          idx += batchSize;
          setProgressLoaded(Math.min(idx, withDist.length));
          if (idx < withDist.length) setTimeout(addBatch, 30);
        };
        addBatch();
      } catch (err) {
        if (err.name !== "AbortError") setError("Could not find mosques. Please try again.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  // Fetch mosques when search location changes (GPS or debounce)
  useEffect(() => {
  ...
  }, [searchLocation, searchMosques, userLocation]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="relative">
              ...
              <input
                ...
                onKeyDown={async (e) => {
                  ...
                  setError(null);
                  let lat, lng, label;
                  const city = MAJOR_CITIES.find(c => c.name.toLowerCase() === trimmed.toLowerCase());
                  if (city) {
                    ...
                    try {
                      ...
                      lat = geoCoords.lat; lng = geoCoords.lng; label = geoCoords.displayName;
                    } catch (err) {
                      if (err.name !== "AbortError") setError(`Geocoding failed: ${err.message}`);
                      setGeocoding(false);
                      return;
                    }
                    setGeocoding(false);
                  }
                  searchedLocationRef.current = { lat, lng };
                  setSearchLocation({ lat, lng });
                  setSearchLocationLabel(label);
                  setLoading(true);
                  try {
                    const results = await searchMosquesViaOverpass({ lat, lng }, radius, "", new AbortController().signal);
                    const withDist = results.map(m => ({
                      ...m,
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* Results (always visible while loading new ones for smoothness) */}
        {filteredMosques.length > 0 && (
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              {(loading || progressLoaded < progressTotal) && (
                <div className="flex items-center gap-2 text-xs text-emerald-600">
                  <Loader2 className="w-3 h-3 animate-spin" />
                  <span>{progressLoaded < progressTotal
                    ? (isRtl ? "لوڈ ہو رہا ہے..." : "Loading...")
                    : (isRtl ? "اپ ڈیٹ ہو رہا ہے..." : "Updating...")}
                  </span>
                </div>
              )}
            </div>
